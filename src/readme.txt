...
五，策略模式
定义了一组算法，将每个算法都封装起来，并且每一个算法可以切换。增加一个策略，就增加一个类就好。
一般出现在if-else判断比较多的情况下，可以使用策略模式代理。

现实例子：出去旅游坐飞机，坐动车，自己开车等等交通方式都可以使用策略模式。支付场景中也是使用策略模式：微信支付，
支付宝支付，京东白条等支付方式，客户就选择其中的任何一条支付方式即可，每一个算法都是独立封装的，每一个算法
都可以切换。线程池里面的构造方法，其中我们可以找到RejectedExecutionHandler，
这个参数代表的是拒绝策略(有四种具体的实现：直接抛出异常、使用调用者的线程来处理、直接丢掉这个任务、丢掉最老的任务)
其实这就是策略模式的体现了

策略模式和工厂模式区别：在运行时，两者都是通过传入参数进行配置，简单工厂模式则是选择创建出需要的对象，基于对象封装；
而策略模式则是配置出需要的行为算法，基于行为封装。
http://www.imooc.com/article/270302

六，模板模式
一个操作中的算法的骨架，而将一些步骤延迟到子类中，模板方法使得子类只可以不改变一个算法的结构即可重定义该算法的某些特定步骤。
通俗一点说明：我们先定义一套固定的规则标准，然后让不确定的方法设置为抽象的，等到具体的实现的时候，再让子类去实现。
先设定好一套规则，定义好一套业务流程，再去具体实现。比如我们新员工入职，盖房子等，他们都是先预先定制好了一套流程，我们按照这个流程做下即可。

总的来说，模板模式就是通过抽象类来定义一个逻辑模板，逻辑框架、逻辑原型，然后将无法决定的部分抽象成抽象类交由子类来实现，
一般这些抽象类的调用逻辑还是在抽象类中完成的。这么看来，模板就是定义一个框架，比如盖房子，我们定义一个模板：房子要封闭，有门，有窗等等，
但是要什么样的门，什么样的窗，这些并不在模板中描述，这个交给子类来完善，比如门使用防盗门，窗使用北向的窗等等。
...
原来有登录的操作（只有用户名和密码，但是运行良好了），新增新的登录操作，比如qq登录，微信登录。这些登录操作不要去影响原来的登录业务。
这个时候就需要适配器模式。新建新的登录模式，比如qqLogin ，weChatLogin 这些登录继承原来的登录类，实现自身的登录方法即可。
...
AOP:面向切面编程，特点：增强效果（汉堡的几层），解耦（螺丝，螺帽），规则组合（汽车，轮胎）
OOP:（封装，继承，多态）面向对象编程，描述的是事物之间的联系，而不是事物的实现过程（面向过程编程）--
    拿到需求，就开始干还是拿到需求，先进行分析他们之间的关系
BOP:面向bean编程。所谓的bean就是普通的类。bean与bean之间的关系，不希望每次人为的去创建他，而是由程序去自动控制。
    所以spring出现在之后，就有了bean的管理。

IOC：bean是如何管理的？控制反转。创建对象的控制权反转了。没有spring之前是谁使用谁new出来的，
    有了spring之后，所有的bean都有spring来new，所以才叫控制反转。new出来的对象需要spring来统一管理，所以才了ioc容器（map）

DI:解决对象之间的依赖关系，解决对象之间的赋值问题，动态调用get，set方法（基本上使用了反射技术）


ioc：从主线业务的角度来说，当下就是在做这个主要的业务，针对这个业务周边的事情，是不需要考虑的，所以从主线业务来说，
